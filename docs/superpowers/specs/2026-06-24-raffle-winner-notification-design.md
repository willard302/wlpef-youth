...
- 日期：2026-06-24
- 範圍：**後端為主**（資料表、DB 函式、輪詢端點、快取與 gating 策略）。前端動畫與 UI 原為後續另開。
- 狀態：本設計為原始後端設計文件（歷史）。後端 + 前端（全域通知、雙層 gating、後台控制台 `/admin/raffle`、測試頁）皆已實作完成，**目前完整功能文件以 `docs/raffle-feature.md` 為準**。
