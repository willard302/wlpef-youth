...
alter table "public"."event_registrations" add column "demo_user" boolean default false;
...
alter table "public"."raffle_winners" drop column "round";
alter table "public"."raffle_winners" add column "prize" text not null;
CREATE INDEX idx_event_registrations_pending_invitation ON public.event_registrations USING btree (matched_user_id, demo_user, invitation_sent_at) WHERE ((matched_user_id IS NULL) AND (demo_user = true) AND (invitation_sent_at IS NULL));
CREATE INDEX idx_raffle_winners_event_round ON public.raffle_winners USING btree (event_id, prize);
