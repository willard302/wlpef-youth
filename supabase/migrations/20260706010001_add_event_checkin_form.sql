ALTER TABLE public.events
  ...
  ADD COLUMN IF NOT EXISTS checkin_form_google_id TEXT,
  ADD COLUMN IF NOT EXISTS checkin_response_sheet_id TEXT,
  ADD COLUMN IF NOT EXISTS checkin_form_bonus_points INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS checkin_visibility_mode TEXT NOT NULL DEFAULT 'test';
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'events_checkin_visibility_mode_check'
      AND conrelid = 'public.events'::regclass
  ) THEN
    ALTER TABLE public.events
      ADD CONSTRAINT events_checkin_visibility_mode_check
      CHECK (checkin_visibility_mode IN ('test', 'live'));
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'events_checkin_form_bonus_points_check'
      AND conrelid = 'public.events'::regclass
  ) THEN
    ALTER TABLE public.events
      ADD CONSTRAINT events_checkin_form_bonus_points_check
      CHECK (checkin_form_bonus_points >= 0);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_events_checkin_response_sheet_id
  ON public.events (checkin_response_sheet_id)
  WHERE checkin_response_sheet_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS public.event_checkin_responses (
  ...
  checkin_form_points_granted_at TIMESTAMPTZ,
  raw_data JSONB,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE (event_id, email)
);

CREATE INDEX IF NOT EXISTS idx_event_checkin_responses_matched_user_id
  ON public.event_checkin_responses (matched_user_id)
  WHERE matched_user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_event_checkin_responses_pending_points
  ON public.event_checkin_responses (event_id, matched_user_id)
  WHERE checkin_form_points_granted_at IS NULL AND matched_user_id IS NOT NULL;

ALTER TABLE public.event_checkin_responses ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view own checkin form responses" ON public.event_checkin_responses;
CREATE POLICY "Users can view own checkin form responses"
  ON public.event_checkin_responses FOR SELECT
  USING (auth.uid() = matched_user_id);

DROP POLICY IF EXISTS "Admins can view all checkin form responses" ON public.event_checkin_responses;
CREATE POLICY "Admins can view all checkin form responses"
  ON public.event_checkin_responses FOR SELECT
  USING (public.is_admin(auth.uid()));

DO $$
BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE public.event_checkin_responses;
EXCEPTION
...
BEGIN
  ...
  LOOP
    INSERT INTO public.point_transactions (user_id, event_id, points, type, description)
    VALUES (r.user_id, r.event_id, r.points, 'bonus', '打卡表單完成獎勵: ' || r.event_title);
